...
export class RestApiServer {
  ...
  constructor(private readonly opts: RestApiServerOpts, modules: RestApiServerModules) {
    ...
    server.addHook("onRequest", async (req, res) => {
      ...
      console.debug(`Req ${req.id} ${req.ip} ${operationId}`);
    ...
    server.addHook("onResponse", async (req, res) => {
      const {operationId} = res.context.config as RouteConfig;
      console.debug(`Res ${req.id} ${operationId} - ${res.raw.statusCode}`);
    ...
    server.addHook("onError", async (req, res, err) => {
      ...
      const {operationId} = res.context.config as RouteConfig;
      console.error(`Req ${req.id} ${operationId} error`, {}, err);
  ...
  async listen(): Promise<void> {
    try {
      ...
      console.log("E ", e);
      console.error("Error starting REST api server", this.opts, e as Error);
      throw e;
    }
  }
