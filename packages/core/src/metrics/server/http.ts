...
import {wrapError} from "../../lib/wrapError.js";
...
export const defaultMetricsServerOpts = {
  port: 8008,
  host: '127.0.0.1'
}
...
export class HttpMetricsServer {
  ...
  constructor(private readonly opts: HttpMetricsServerOpts, {register}: {register: Registry}) {
  ...
  async start(): Promise<void> {
    ...
    console.info("Starting metrics HTTP server", { address: `http://${address ?? "127.0.0.1"}:${port}`});
  ...
  private async onRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    if (req.method === "GET" && req.url && req.url.includes("/metrics")) {
      ...
      if (metricsRes.err) {
        ...
        const metricsStr = `${(metricsRes as any).result}\n\n${httpServerMetrics}`;
