...
import {DeletionStatus, ImportStatus, PubkeyHex } from "@lodestar/api/keymanager";
import {
...
export class PersistedKeysBackend implements IPersistedKeysBackend {
  ...
  writeKeystore({
    ...
    writeFileSync(keystoreFilepath, keystoreStr);
    writeFileSync(passphraseFilepath, password);
